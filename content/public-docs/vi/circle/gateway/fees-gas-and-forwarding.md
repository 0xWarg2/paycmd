...
aiSummary:
  ...
  - "Payna quote trước state change, ưu tiên fees.total của Circle và xử lý forwarded/manual destination hash theo hai result shape khác nhau."
---

## Bốn loại cost cần tách biệt

Một Gateway preview có thể liên quan bốn cost category:
...
Nếu legacy manual quote không có usable `fees.total`, Payna có thể fallback sang `burnIntent.maxFee` đầu tiên như positive atomic reserve. Response label hai trường hợp `quoted_total` và `max_fee_reserve`. Zero, malformed hoặc absent value đều fail closed; Payna không tự tạo zero fee.
...
Payna estimate trước signer creation, delegate authorization, auto-deposit hoặc burn signing. Ordering này khiến quote failure là read-only failure và ngăn unavailable preview để lại wallet hay balance mutation.

Preview phải expose `estimatedGatewayFee`, `requiredGatewayBalance`, `feeEstimateKind`, mint mode và forwarding state. Nó không được hiển thị hard-coded fixed fee trước khi estimate trả về. Nó cũng không được gọi estimate là “actual”. Khi user đổi amount, source, destination, recipient hoặc mint mode, cần quote mới.
...
Các operation đó cần native gas trên source SCA và current Circle Wallet SDK support cho chain. SCA có thể đủ USDC nhưng vẫn fail `INSUFFICIENT_GAS`. Chỉ fund public address và network được error nêu, sau đó lấy fresh preview. Không gửi native gas tới depositor contract hoặc signer trừ khi response xác định nó là transaction sender.
...
Với cả hai mode, kiểm tra source, destination, recipient, source debit và quote type. `/gas <chain>` giúp inspect balance nhưng execution preview mới authoritative về wallet role nào cần gas. Same-chain route không tự bị ép sang manual; Payna giữ selected mint mode.

## Failure và retry checklist

- Quote unavailable: chưa nên có stateful work; lấy fresh estimate sau.
